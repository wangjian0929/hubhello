...
import static com.feedAustralia.android.utilCalender.MonthIndex.FOCUSED_MONTH;
import static com.feedAustralia.android.utilCalender.MonthIndex.NEXT_MONTH;
import static com.feedAustralia.android.utilCalender.MonthIndex.PREVIOUS_MONTH;
...
public class CalendarView extends View {
     ...
     */
    private static final float RATIO_ROW_HEIGHT_WIDTH = 0.110f;
    ...
    // Listeners
    private OnDateSelectedListener mOnDateSelectedListener,mOnDateSelectedListener_day;;
    ...
    private float mPlaceForPointsWidth,velocity; // Place where events points are located
    ...
    public CalendarView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            ...
            try {
                ...
                mTextSize = (int) typedArray.getDimensionPixelSize(R.styleable.CalendarView_textsize,60);
                mTextColor = typedArray.getColor(R.styleable.CalendarView_textColor,getResources().getColor(R.color.green));
    ...
    public void init() {
        ...
        if(mMonthPager== null)
        mMonthPager = new MonthPager(2,selectedCalendar);
...
//                }
                ...
                dispatchOnDateSelected_day(calendarMonth.getCalendar(),day,calendarMonth.getMonth(),calendarMonth.getYear(),calendarMonth.getEventOfDay(day));
                return true;
            }
        ...
        float scaledSizeInPixels = getResources().getDimensionPixelSize(R.dimen._16sdp);
        mTextPaint.setTextSize(scaledSizeInPixels);
        Typeface plain = Typeface.createFromAsset(getContext().getAssets(),"avenirltstd_book.ttf");
        ...
        mWeekDaysNamesTextPaint.setTypeface(plain);
    ...
    private void drawMonth(Canvas canvas, @MonthIndex int monthIndex) {
        ...
        if(mMonthPager.getCurrentDay() == mMonthPager.getSelectedDay())
        {
        ...
        for (int day = 1; day <= calendarMonth.getAmountOfDays(); day++) {
            ...
            canvas.drawText(Integer.toString(day), crd[0], crd[1],
                  isSelectedDay ? mTextInsideCirclePaint : mTextPaint);
    ...
    private void drawEventsOfDay(Canvas canvas, List<CalendarEvent> events, float[] crd, int day) {
        ...
        if(mMonthPager.getSelectedDay() == day)
        {
            ...
            for (int i = 0; i < events.size(); i++) {
                mEventCirclePaint.setColor(getResources().getColor(R.color.white));
    ...
    public void handleGesture(float mvelocity) {
        velocity=mvelocity;
        if (velocity == 0.0 && mOffset == 0) {
            return;
        }else
        {
    ...
    /**
     * Change view_ height, for next month
    ...
    private void dispatchOnDateSelected_day(Calendar calendar, int eventsOfDay, int month, int year, List<CalendarEvent> eventOfDay) {
        if (mOnDateSelectedListener != null) {
            mOnDateSelectedListener.onDateSelected_day(calendar, eventsOfDay,month,year,eventOfDay);
        }
    }
    ...
    public void setOnDateSelectedListener(OnDateSelectedListener onDateSelectedListener) {
        ...
        mOnDateSelectedListener_day=onDateSelectedListener;
